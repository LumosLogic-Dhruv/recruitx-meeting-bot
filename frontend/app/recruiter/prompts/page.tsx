...
function EditPromptModal({ prompt, onClose, onSaved }: { prompt: Prompt; onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <div onClick={e => { if (e.target === e.currentTarget) onClose(); }} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.65)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 20, backdropFilter: "blur(4px)" }}>
      <div style={{ background: "var(--bg-2)", backdropFilter: "blur(24px)", border: "1px solid var(--border)", borderRadius: 16, width: "100%", maxWidth: 600, padding: 32, maxHeight: "90vh", overflowY: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <h3 style={{ fontSize: 18, fontWeight: 700, margin: 0, color: "var(--text)" }}>Edit Prompt</h3>
        ...
        <form onSubmit={save}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <button type="button" onClick={onClose} style={{ flex: 1, padding: "10px", background: "var(--sunken)", color: "var(--text)", border: "1px solid var(--border)", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Cancel</button>
...
export default function PromptsPage() {
  ...
  return (
    <>
      <h1 style={{ fontSize: 22, fontWeight: 800, color: "var(--text)", margin: "0 0 24px" }}>Generate Prompt</h1>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, alignItems: "start" }}>

        {/* Generator */}
        <div style={card}>
          <h2 style={{ fontSize: 15, fontWeight: 700, color: "var(--text)", margin: "0 0 4px" }}>AI Prompt Engineer</h2>
          <p style={{ fontSize: 12, color: "var(--text-muted)", margin: "0 0 18px" }}>Generate a tailored interviewer prompt from a role name or uploaded documents.</p>

          {/* Tab toggle */}
          <div style={{ display: "flex", gap: 0, background: "var(--sunken)", border: "1px solid var(--border)", borderRadius: 10, padding: 4, marginBottom: 20 }}>
          ...
          {tab === "role" ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <p style={{ fontSize: 11, color: "var(--text-muted)", margin: 0 }}>Upload at least one document.</p>
        ...
        {/* Library */}
        <div style={card}>
          <h2 style={{ fontSize: 15, fontWeight: 700, color: "var(--text)", margin: "0 0 4px" }}>Saved Prompt Library</h2>
          <p style={{ fontSize: 12, color: "var(--text-muted)", margin: "0 0 16px" }}>Quickly reuse prompts for standard roles.</p>
          <div style={{ maxHeight: 600, overflowY: "auto" }}>
            {libError ? (
              <p style={{ color: "#f87171", textAlign: "center", padding: 24 }}>Failed to load prompts.</p>
            ) : library.length === 0 ? (
              <p style={{ color: "var(--text-muted)", textAlign: "center", padding: 24 }}>No saved prompts yet.</p>
            ) : library.map(p => (
              <div key={p._id} style={{ background: "var(--sunken)", border: "1px solid var(--border)", borderRadius: 10, padding: "14px 16px", marginBottom: 10 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "var(--text)", marginBottom: 4 }}>{p.roleName}</div>
