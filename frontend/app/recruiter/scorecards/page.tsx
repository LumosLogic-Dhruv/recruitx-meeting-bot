...
function StatusBadge({ c }: { c: Candidate }) {
  ...
  const m: Record<string, [string, string, string]> = {
    never_invited:        ["var(--surface-3)", "var(--text-muted)", "Not Invited"],
  ...
  const [bg, col, label] = m[s] || ["var(--surface-3)", "var(--text-muted)", s.replace(/_/g, " ")];
      ...
      <div className="responsive-4-grid" style={{ marginBottom: 24 }}>
        {stats.map(s => (
          <div key={s.label} style={{ ...card, padding: 20, textAlign: "center" }}>
            ...
            <p style={{ margin: "4px 0 0", fontSize: 11, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: ".06em" }}>{s.label}</p>
          </div>
        ))}
      </div>

      <div style={{ ...card, padding: 28, overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 600 }}>
          <thead>
            <tr>
              {["Candidate", "Role", "Attempts", "Best Score", "Recommendation", "Status", "Scorecards"].map(h => (
                <th key={h} style={{ textAlign: "left", padding: "10px 13px", fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".06em", color: "var(--text-muted)", background: "var(--sunken)", borderBottom: "2px solid var(--border)" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {candidates.length === 0 ? (
              <tr><td colSpan={7} style={{ textAlign: "center", color: "var(--text-muted)", padding: 40 }}>No candidates yet. <a href="/recruiter/add" style={{ color: "#a78bfa" }}>Add one →</a></td></tr>
            ) : candidates.map(c => {
              ...
              return (
                <tr key={c._id}>
                  <td style={{ padding: 13, fontSize: 13, borderBottom: "1px solid var(--border)" }}>
                    <strong style={{ color: "var(--text)" }}>{c.name}</strong><br />
                    <span style={{ fontSize: 11, color: "var(--text-muted)" }}>{c.email}</span>
                  </td>
                  <td style={{ padding: 13, fontSize: 13, borderBottom: "1px solid var(--border)", color: "#e2e8f0" }}>{c.roleName || best?.roleName || "—"}</td>
                  <td style={{ padding: 13, fontSize: 13, textAlign: "center", borderBottom: "1px solid var(--border)", color: "var(--text-muted)" }}>{c.attemptCount || 0}/2</td>
                  <td style={{ padding: 13, borderBottom: "1px solid var(--border)" }}>
                    {best?.scorecard?.overall_score ? <ScoreChip score={best.scorecard.overall_score} /> : <span style={{ color: "var(--text-muted)" }}>—</span>}
                  </td>
                  <td style={{ padding: 13, fontSize: 12, color: "var(--text-muted)", borderBottom: "1px solid var(--border)" }}>{best?.scorecard?.recommendation || "—"}</td>
                  <td style={{ padding: 13, borderBottom: "1px solid var(--border)" }}><StatusBadge c={c} /></td>
                  <td style={{ padding: 13, borderBottom: "1px solid var(--border)" }}>
                    {cMeetings.length > 0 ? (
                    ...
                    ) : <span style={{ color: "var(--text-muted)", fontSize: 12 }}>No results yet</span>}
                  </td>
                </tr>
              );
            })}
